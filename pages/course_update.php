...
if(isset($_POST['update'])){
   ...
   $update_listing = $conn->prepare("UPDATE `course` SET institute=? ,course_name=?, description=?, university=?, premises=?, location=?, address=?, distance=?, duration=?, fees=?, prerequisites=?, scheduling=?, contact_information=?,certificate=?, image_01=?,image_02=?, date=? WHERE id = ?");   
   $update_listing->execute([$institute, $course_name, $description, $university, $premises, $location, $address, $distance,  $duration, $fees, $prerequisites, $scheduling, $contact_information, $certificate, $rename_image_01, $rename_image_02, $date,  $update_id]);
   ...
   <form action="" method="POST" enctype="multipart/form-data">
      ...
      <div class="flex">
         ...
         <div class="box">
            ...
            <input type="text" name="location" maxlength="100" placeholder="Location" class="input" value="<?= $fetch_course['location']; ?>">
         </div>
         ...
         <textarea name="description" maxlength="1000" class="input" required cols="30" rows="10" placeholder="write about property..." ><?= $fetch_course['description']; ?></textarea>
      </div>
         <br>
