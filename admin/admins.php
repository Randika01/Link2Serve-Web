...
if(isset($_POST['delete'])){
   ...
   $select_main_admin = $conn->prepare("SELECT * FROM `admins` WHERE id = ?");
   $select_main_admin->execute([$admin_id]);
   $main_admin = $select_main_admin->fetch(PDO::FETCH_ASSOC);

   // Ensure only the main admin can delete other admins
   if ($main_admin['is_main_admin'] == 1) {
      $verify_delete = $conn->prepare("SELECT * FROM `admins` WHERE id = ?");
      $verify_delete->execute([$delete_id]);

      if($verify_delete->rowCount() > 0){
         // Check if the admin to be deleted is not the main admin
         $admin_to_delete = $verify_delete->fetch(PDO::FETCH_ASSOC);
         if ($admin_to_delete['is_main_admin'] == 1) {
            $warning_msg[] = 'Cannot delete the main admin!';
         } else {
      ...
      $warning_msg[] = 'You do not have permission to delete admins!';
   }
}
