...
<section class="dashboard">
   ...
   <div class="box">
   <?php
      // Fetch the count of boardingss
      $select_properties = $conn->prepare("SELECT * FROM `property`");
      $select_properties->execute();
      $count_properties = $select_properties->rowCount();

      // Fetch the count of jobs
      $select_jobs = $conn->prepare("SELECT * FROM `job`");
      $select_jobs->execute();
      ...
      // Fetch the count of couriers
      $select_couriers = $conn->prepare("SELECT * FROM `courier`");
      $select_couriers->execute();
      $count_couriers = $select_couriers->rowCount();

      // Fetch the count of bikes
      $select_bikes = $conn->prepare("SELECT * FROM `bike`");
      $select_bikes->execute();
      $count_bikes = $select_bikes->rowCount();

      // Total count of listings
      $total_listings = $count_properties + $count_jobs + $count_courses + $count_couriers + $count_bikes;
   ?>
      <h3><?= $total_listings; ?></h3>
      <p>All Listings posted</p>
